...
for group in category:
    a = db.itproject_clean.find(
        {"region": {"$ne": None}, "bereich.group": group}
    ).count()
    lengths.append(a)
default = {
    "size": 500,
    "aggs": {
        "Group": {"terms": {"field": "bereich.group.keyword", "size": 10}},
        "Group Type": {"terms": {"field": "bereich.group_type.keyword", "size": 10}},
        "Group Stack": {
            "terms": {"field": "bereich.group_type_stack.keyword", "size": 10}
        },
        "Skill Filter": {"terms": {"field": "skill_summary.keyword", "size": 10}},
        "Region Filter": {"terms": {"field": "region.bundesland.keyword", "size": 10}},
    },
}
...
def register():
    ...
    # insert user in db
    users.insert({"Username": username, "Password": hashedpw})
...
def api(user):
    ...
    if (
        ...
        and not groupStack
        and not bundesland
        and not skill_summary
        and not skill
        and not platform
        and not platform_name
        and not search_term
    ):
        if "sort" in default:
            del default["sort"]
        default["query"] = {"bool": {"must": [{"match": {"bereich.group": group}}]}}
        body = default
        print(1)
        ...
        default["query"] = {
            "bool": {
                "must": [{"match": {"bereich.group": group}}],
        ...
        if "sort" in default:
            del default["sort"]
        default["query"] = {
            "bool": {"must": [{"match": {"skill_summary.keyword": skill_summary}}]}
        }
        body = default
        print(9)
    elif (
        ...
        default["query"] = {
            "bool": {
                "must": [{"match": {"skill_summary.keyword": skill_summary}}],
                "filter": {"term": {"region.bundesland.keyword": bundesland}},
            }
        }
        body = default
        ...
        skill_summary and groupStack and not bundesland and not group and not groupType
    ):
        if "sort" in default:
            del default["sort"]
        default["query"] = {
        ...
        if "sort" in default:
            del default["sort"]
        default["query"] = {
            "bool": {"must": [{"match": {"bereich.group_type": groupType}}]}
        }
        body = default
        print(17)
    elif (
        ...
        default["query"] = {
            "bool": {
                "must": [{"match": {"bereich.group_type": groupType}}],
                "filter": {"term": {"region.bundesland.keyword": bundesland}},
            }
        }
        body = default
        ...
        if "sort" in default:
            del default["sort"]
        default["query"] = {
            "bool": {"must": [{"match": {"bereich.group_type_stack": groupStack}}]}
        }
        body = default
        print(19)
    elif (
        ...
        default["query"] = {
            "bool": {
                "must": [{"match": {"bereich.group_type_stack": groupStack}}],
                "filter": {"term": {"region.bundesland.keyword": bundesland}},
            }
        }
        body = default
        ...
        if "sort" in default:
            del default["sort"]
        default["query"] = {"bool": {"must": [{"match": {"bereich.skill": skill}}]}}
        body = default
        print(21)
    elif (
        skill
        ...
        if "sort" in default:
            del default["sort"]
        default["query"] = {
            "bool": {"must": [{"match": {"region.bundesland": bundesland}}]}
        }
        body = default
        print(23)
    elif (
        ...
        default["sort"] = [{"_score": {"order": "desc"}}]
        body = default
        print(299)
    else:
        if "query" in default:
            del default["query"]
        default["sort"] = [{"filter_date_post": {"order": "desc"}}, "_score"]
        body = default
        """body = {
                "size" : 500,
                "sort": [
    ...
    result = es.search(index="projectfinder", doc_type="itproject_clean", body=body)
    print(result["hits"]["total"])
    try:
        # clean up
        docs = [
            {"source": doc["_source"], "score": doc["_score"], "id": doc["_id"]}
            for doc in result["hits"]["hits"]
            if "region" in doc["_source"]
        ]
    except KeyError:
        # return message
    ...
    projects = [
        {
            ...
            "filter_date_post": datetime.strptime(
                hit["source"]["filter_date_post"], "%Y-%m-%dT%H:%M:%S"
            )
            if hit["source"]["filter_date_post"]
    ...
    regionAgg = [
        {
            ...
            "path": "&bundesland=",
            "items": [
                {"key": hit["key"], "count": hit["doc_count"]}
